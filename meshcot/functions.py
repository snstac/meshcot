...
def create_tasks(config: SectionProxy, clitool: pytak.CLITool) -> Set[pytak.Worker,]:
    ...
    tasks.add(meshcot.MQTTWorker(net_queue, config))
    tasks.add(meshcot.MeshWorker(clitool.tx_queue, net_queue, config))
...
def xml_to_cot(
    ...
    cot: Optional[ET.Element] = getattr(meshcot.functions, func)(data, config)
    return (
...
def get_gps_info(config) -> Optional[dict]:
    ...
    gps_info_cmd = config.get("GPS_INFO_CMD", meshcot.DEFAULT_GPS_INFO_CMD)
